...
TranslationEntry*  AddrSpace::getPagetable(int vpn)
{
...
TranslationEntry*  AddrSpace::getByPPN(int PPN)
{
   for (int i = 0; i<numPages;i++)
   {
...
AddrSpace::Load(char *fileName) 
{
    ...
    int a  = executable->ReadAt((char *)&noffH, sizeof(noffH), 0);
...
// Note: this code assumes that virtual address = physical address
    ...
    bzero(buff, MemorySize); 
    if (noffH.code.size > 0) {
        DEBUG(dbgAddr, "Initializing code segment.");
	DEBUG(dbgAddr, noffH.code.virtualAddr << ", " << noffH.code.size);
         executable->ReadAt(
                 &(buff[noffH.code.virtualAddr]),noffH.code.size, noffH.code.inFileAddr);
    }
	...
	DEBUG(dbgAddr, noffH.initData.virtualAddr << ", " << noffH.initData.size);
        executable->ReadAt(
		&(buff[noffH.initData.virtualAddr]),
	...
	DEBUG(dbgAddr, noffH.readonlyData.virtualAddr << ", " << noffH.readonlyData.size);
        executable->ReadAt(
		&(buff[noffH.readonlyData.virtualAddr]),
	...
	pageTable[i].dirty = FALSE;
	pageTable[i].readOnly = FALSE;  
	pageTable[i].swapLocation = kernel->getSwapLocation();
        ...
        int buffStart = i * PageSize;
        for(int k = 0; k < PageSize; k++){

                 temp[k] = (buff[buffStart++]);
         }

	kernel->getSwapFile()->WriteAt(temp,PageSize,(pageTable[i].swapLocation * PageSize));
    }
    delete executable;
    return TRUE;			// success
}
